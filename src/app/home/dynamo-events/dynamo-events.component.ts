...
@Component({
  selector: 'app-dynamo-events',
  templateUrl: './dynamo-events.component.html',
  styleUrls: ['./dynamo-events.component.scss'],
})
export class DynamoEventsComponent implements OnInit {
  ...
  toggleEventActiveStatus(event: any, eventObj: any) {
    const newStatus = event.detail.checked ? 'ACTIVE' : 'UPCOMING';
    eventObj.status = newStatus;
    this.firestore.collection('events').doc(eventObj.id).update({
      status: newStatus
    }).then(() => {
      this.presentToast(`Event ${newStatus === 'ACTIVE' ? 'Activated' : 'Deactivated'} Successfully`);
      // Refresh the list to reflect changes
      this.search();
    }).catch(err => {
      this.presentToast('Failed to update event status');
